...
foreach ($hosts as $h) {
    ...
    // Try to query Information Schema or test database names
    $test_names = [
    ...
    foreach ($test_names as $dbname) {
        if (@$conn->select_db($dbname)) {
            ...
            // Auto update .env and import_db.php
    ...
    if (!$found) {
        echo "<b style='color:orange'>No database found on $h. Database is not created yet in cPanel -> MySQL Databases.</b><br>";
    }
...
print("Uploading check_all_db.php...")
