    ...
    <div class="container">
        <div style="text-align: center; max-width: 800px; margin: 0 auto;">
            ...
            <h1 style="font-size: 3rem; font-weight: 900; margin-bottom: 0.75rem; font-family: 'Outfit', sans-serif; letter-spacing: -0.02em;">
                Blog &amp; <span style="color: #84cc16;">Tips Fitness</span>
            </h1>
            ...
            <div style="max-width: 550px; margin: 0 auto; position: relative;">
                <input type="text" id="blogSearchInput" placeholder="Cari artikel (contoh: Fat Loss, Protein, Squat, TNI)..." value="{{ request('search') }}" style="width: 100%; background: #0d1310; border: 1.5px solid rgba(132, 204, 22, 0.4); padding: 0.95rem 1.25rem 0.95rem 3.1rem; border-radius: 99px; color: white; font-size: 0.95rem; outline: none; font-weight: 700; box-shadow: 0 10px 30px rgba(0,0,0,0.6);" onkeyup="filterBlogPostsLive()" onfocus="this.style.borderColor='#84cc16'" onblur="this.style.borderColor='rgba(132, 204, 22, 0.4)'">
                <i class="fa-solid fa-magnifying-glass" style="position: absolute; left: 1.25rem; top: 50%; transform: translateY(-50%); color: #84cc16; font-size: 1.1rem;"></i>
            </div>
        </div>
    </div>
</section>
...
<section style="padding: 1.5rem 0; background: #090d0b; border-bottom: 1px solid rgba(255,255,255,0.08); sticky: top; z-index: 10;">
    <div class="container">
        <div style="display: flex; gap: 0.75rem; flex-wrap: wrap; justify-content: center; align-items: center;">
            <a href="{{ route('blog.index') }}" class="btn btn-sm" style="background: {{ !request('category') ? '#84cc16' : 'rgba(255,255,255,0.05)' }}; color: {{ !request('category') ? '#090d0b' : '#cbd5e1' }}; border: 1.5px solid {{ !request('category') ? '#84cc16' : 'rgba(255,255,255,0.12)' }}; padding: 0.55rem 1.25rem; border-radius: 99px; font-weight: 800; text-decoration: none; transition: all 0.2s;">
                🌟 Semua Artikel
            </a>
            @foreach($categories as $cat)
                <a href="{{ route('blog.index', ['category' => $cat]) }}" class="btn btn-sm" style="background: {{ request('category') == $cat ? '#84cc16' : 'rgba(255,255,255,0.05)' }}; color: {{ request('category') == $cat ? '#090d0b' : '#cbd5e1' }}; border: 1.5px solid {{ request('category') == $cat ? '#84cc16' : 'rgba(255,255,255,0.12)' }}; padding: 0.55rem 1.25rem; border-radius: 99px; font-weight: 800; text-decoration: none; transition: all 0.2s;">
                    {{ $cat }}
                </a>
            @endforeach
        </div>
...
<section style="background: #060907; padding: 4.5rem 0 6rem; color: white;">
    <div class="container">
            ...
            <div class="blog-card-item" data-title="{{ strtolower($post->title) }}" data-category="{{ strtolower($post->category) }}" data-excerpt="{{ strtolower($post->excerpt) }}" style="overflow: hidden; background: #0d1310; border: 1.5px solid rgba(255,255,255,0.1); border-radius: 1.5rem; display: flex; flex-direction: column; transition: transform 0.3s ease, border-color 0.3s ease;" onmouseover="this.style.transform='translateY(-6px)'; this.style.borderColor='#84cc16';" onmouseout="this.style.transform='translateY(0)'; this.style.borderColor='rgba(255,255,255,0.1)';">
                ...
                <div style="padding: 1.65rem; display: flex; flex-direction: column; flex-grow: 1;">
                    <div style="font-size: 0.775rem; font-weight: 700; color: #94a3b8; margin-bottom: 0.65rem; display: flex; align-items: center; justify-content: space-between;">
                        <span><i class="fa-solid fa-user-pen" style="color: #84cc16;"></i> Coach FitLife</span>
                        <span><i class="fa-regular fa-clock" style="color: #84cc16;"></i> {{ $post->reading_time }} Menit Baca</span>
                    </div>
                    ...
                    <h2 style="font-size: 1.25rem; margin-bottom: 0.85rem; line-height: 1.4; color: #ffffff; font-weight: 900; font-family: 'Outfit', sans-serif;">
                        <a href="{{ route('blog.show', $post->slug) }}" style="text-decoration: none; color: #ffffff; transition: color 0.2s;" onmouseover="this.style.color='#84cc16'" onmouseout="this.style.color='#ffffff'">
                            {{ $post->title }}
                        </a>
                    </h2>
                    ...
                    <a href="{{ route('blog.show', $post->slug) }}" style="font-weight: 900; color: #84cc16; text-decoration: none; font-size: 0.9rem; margin-top: auto; display: inline-flex; align-items: center; gap: 0.4rem;">
        ...
        <div id="noBlogResults" style="display: none; text-align: center; padding: 4rem 1rem; background: #0d1310; border: 1px dashed rgba(255,255,255,0.15); border-radius: 1.5rem;">
            <div style="font-size: 3rem; color: #84cc16; margin-bottom: 1rem;">🔍</div>
